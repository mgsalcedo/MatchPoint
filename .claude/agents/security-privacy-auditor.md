...
1. **PII/location exposure**: trace where User/Provider location and contact info flow — API responses, logs, analytics events, error reports, client-side state. Flag any path where precise data leaks before the point `docs/security-standards.md` allows it (Booking `confirmed` state).
2. **Auth/session boundaries**: verify User-role and Provider-role sessions can't cross-act; check endpoint-level authorization, not just UI-level hiding of actions.
